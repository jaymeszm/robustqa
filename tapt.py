...
class MaskedLMTrainer():
    ...
    def evaluate(self, model, data_loader, return_preds=False):
        device = self.device
        model.eval()
        eval_loss = 0.0
        eval_steps = 0
...
def main():
    ...
    if args.do_train_mlm:
        ...
        train_dataset = get_masked_dataset(args, args.train_datasets, args.train_dir, tokenizer)
        ...
        val_dataset = get_masked_dataset(args, args.train_datasets, args.val_dir, tokenizer)
        val_loader = DataLoader(val_dataset,
                                batch_size=args.batch_size,
                                sampler=SequentialSampler(val_dataset))
        trainer = MaskedLMTrainer(args, log)
        best_loss = trainer.train(model, train_loader, val_loader)
